...
let encryptionTests endianness name =
    testList name [
        let encrypt = Encryption.encrypt endianness
        testCase "Encrypting the same text twice does not yield the same cipher text" <| fun _ ->
            ...
            let cipherText1 =
                encrypt (Some alice) [ Public, snd bob ] plainText
            let cipherText2 =
                encrypt (Some alice) [ Public, snd bob ] plainText
        ...
        testCase "Rountrip with multiple recipients works." <| fun _ ->
            ...
            let encrypted =
                encrypt (Some alice) recipients plainText
                |> Result.failOnError "Encryption failed"
...
[<Tests>]
let be = encryptionTests toBytesBE "EncryptionBE"

[<Tests>]
let le = encryptionTests toBytes "EncryptionLE"
